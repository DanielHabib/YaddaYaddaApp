...
- (void)viewDidLoad {
    ...
    emojiTyped = [defaults objectForKey:@"emojiTyped"];
    
    if(!self.emojiType){
        NSLog(@"Emoji Type not registering Error-!");
...
-(void)AFPost{
    ...
    if(imageData){
        ...
        AFHTTPRequestOperationManager *manager = [[AFHTTPRequestOperationManager alloc] initWithBaseURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://104.131.53.146/users/%@/emojis/%@",phone,emojiTyped]]];
